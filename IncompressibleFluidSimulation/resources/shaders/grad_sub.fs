...
vec4 gradSub(sampler2D texA, sampler2D texB, vec2 coords, float rdx){
	float t = texture(texB, coords + rdx * vec2(0, 1)).x;
	float b = texture(texB, coords + rdx * vec2(0, -1)).x;
	float r = texture(texB, coords + rdx * vec2(1, 0)).x;
	float l = texture(texB, coords + rdx * vec2(-1, 0)).x;
